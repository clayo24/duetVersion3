...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var TopThreeThings: UITextField!
    @IBOutlet weak var GuiltyPleasure: UITextField!
    @IBOutlet weak var TopThreeGenres: UITextField!
    @IBOutlet weak var TopThreeArtists: UITextField!
    ...
    @IBAction func SignUp(_ sender: UIButton) {
        ...
        if let email = UsernameSignUp.text, let password = PasswordSignUp.text
        {
            Auth.auth().createUser(withEmail: email, password: password) { authResult, error in
                ...
                else
                {
                    ...
                    if let email2 = Auth.auth().currentUser?.email
                    {
                        self.db.collection("User_data").addDocument(data: [
                            "Screen Name": self.ScreenName.text,
                            "Bio": self.Bio.text,
                            "Three things to bring to a concert": self.TopThreeThings.text,
                            "Guilty Pleasure Song": self.GuiltyPleasure.text,
                            "Top 3 Genres": self.TopThreeGenres.text,
                            "Top 3 Artists": self.TopThreeArtists.text,
                    ...
                    let currentAc = Auth.auth().currentUser?.email
                }
            }
        }
  ...
  //log in func
    @IBAction func ButtonLogin(_ sender: UIButton) {
        if let email = UsernameSignIn.text, let password = PasswordSignIn.text
        {
